...
String(format: "%2x%2x%2x%2x", buffer[3], buffer[2], buffer[1], buffer[0])
//: Yep, that looks like a valid (64-bit, little-endian) Mach-O header to me! Let's get this off the device.
...
//: ### Execution
//: So after all this, how does ExecutionExtension actually *execute* our code? A quick look at the disassembly (which, unfortunately, must be done off-device) will show that it calls `dlopen`, then `dlsym`s for "main", which it then executes in some way. Can we see this in a backtrace?
Thread.callStackSymbols
//: There's only one frame from ExecutionExtension here and a bunch from CoreFoundation. What's going on? It turns out that's just `CFRunLoop` stuff; the code that actually calls does the `dlsym` is gone because it was on a previous run loop iteration. We're going to have to attack this some other way. We have multiple methods in the ExecutionExtension binary–what are they? The symbols are stripped, so we're not going to be able to get them statically. Luckily the Objective-C runtime can help us here:
...
//: Now we can cross reference our binary we dumped earlier; we have an offset into it. The number certainly looks about right: it's small, smaller than the size of ExecutionExtension, which means it must be pointing inside our binary (as opposed to `_objc_msgForward` in libobjc.dylib). A quick check suffices to show that it's the function with `dlopen` we found earlier.
//: ### Other functionality
//: It's worth taking a quick look at the other functionality that is necessary for execution to work correctly. `PGEExecutionExtensionViewController` seems to be the main point of contact with the rest of the app, since it implements `NSXPCListener`. It also handles the reporting of execution progress, as well as managing the live view. Line-by-line step through is done in an interesting way: it's a combined effort by both the compiler and the execution extensions. When compiling, calls to `_pgr_pc_before` and `_pgr_pc_after` get automatically inserted between each statement, and these functions (which are defined in the _PlaygroundRuntime framework) are then set to whatever they need to be. They're available for use directly, as `@convention(c) (Int, Int, Int, Int) -> ()` functions, but I haven't been able to get them to do anything useful yet.
_pgr_pc_before
_pgr_pc_after
